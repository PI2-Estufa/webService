...
@app.route('/drawer', methods=['GET'])
def drawer_control():
    drawer_exchange = Exchange('rdrawer', type='direct')
    # task_queues = [Queue('rdrawer', drawer_exchange, routing_key='rdrawer')]
    connection = Connection(os.environ.get('RABBIT_URL'))
    command = '0'
    payload = {'command': command}
    routing_key = 'rdrawer'
    with producers[connection].acquire(block=True) as producer:
        producer.publish(payload,
                        exchange=drawer_exchange,
                        declare=[drawer_exchange],
